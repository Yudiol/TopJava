...
public class JdbcMealRepository implements MealRepository {
    ...
    public Meal save(Meal meal, int userId) {
        ...
        if (meal.isNew()) {
            ...
            meal.setId(newKey.intValue());
                ...
                "UPDATE meals SET description=:description, date_time=:date_time, calories=:calories WHERE id=:id AND user_id=:user_id", map) == 0) {
            return null;
        }
        return meal;
    }
    ...
    public Meal get(int id, int userId) {
        return jdbcTemplate.query("SELECT * FROM meals WHERE id = ? AND user_id = ?",
                new Object[]{id, userId}, ROW_MAPPER).stream().findAny().orElse(null);
    }
    ...
    public List<Meal> getBetweenHalfOpen(LocalDateTime startDateTime, LocalDateTime endDateTime, int userId) {
        return jdbcTemplate.query("SELECT * FROM meals WHERE user_id = ? AND date_time > ? AND date_time < ? ORDER BY date_time DESC",
                new Object[]{userId, startDateTime, endDateTime}, ROW_MAPPER);
    }
}